...
def test_xxd(u_boot_console):
    ...
    try:
        ...
        xxd_data = fs_helper.mk_fs(u_boot_console.config, 'vfat', 0x100000,
                                   'test_xxd', scratch_dir)
        response = u_boot_console.run_command_list([ f'host bind 0 {xxd_data}',
                                                    'xxd host 0 hello'])
